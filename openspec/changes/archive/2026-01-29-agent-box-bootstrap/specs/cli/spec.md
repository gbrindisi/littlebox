...
#### Scenario: Create default Agentfile
- **WHEN** user executes `agentbox init`
- **THEN** the CLI creates an Agentfile with sensible defaults for claude-code profile

#### Scenario: Init with profile
- **WHEN** user executes `agentbox init --profile aider`
- **THEN** the CLI creates an Agentfile configured for the specified agent profile
