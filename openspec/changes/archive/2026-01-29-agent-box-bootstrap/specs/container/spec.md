...
The container manager SHALL build images locally when not cached.

#### Scenario: First run builds image
- **WHEN** user runs agentbox for the first time
- **THEN** container manager builds the image from embedded Dockerfile

#### Scenario: Cached image used
- **WHEN** image with matching version tag exists locally
...
The container manager SHALL inject environment variables into the container.

#### Scenario: Passthrough variables
- **WHEN** profile specifies env_passthrough variables
